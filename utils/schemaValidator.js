...
const casosFilterSchema = z.object({
    status: z
        .enum(['solucionado', 'aberto'], "O parâmetro 'status' deve ser 'solucionado' ou 'aberto'.")
        .optional(),
    agente_id: z.uuid("O parâmetro 'agente_id' deve ser um UUID válido.").optional(),
});

const agentesFilterSchema = z.object({
    cargo: z.string().min(1, "O parâmetro 'cargo' deve ser uma string não vazia.").optional(),
    sort: z
